...
# Initialize the OpenAI language model and memory
llm = OpenAI(temperature=0.5, openai_api_key="YOUR_OPENAI_API_KEY")
...
while True:
    ...
    if user_input:
        ...
        conversation_with_memory.add_input(user_input)

        # Generate a response from the chatbot
        bot_response = conversation_with_memory.predict(input=user_input)

        # Add the bot response to the conversation history
        conversation_with_memory.add_output(bot_response)

        # Display the conversation history
        conversation_history = conversation_template.render(conversation_with_memory.history)
        st.text_area("Conversation History", value=conversation_history, height=400)
